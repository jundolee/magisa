...
**결정**: 홈 화면을 다시 기본 Node.js 런타임으로 되돌림(`runtime`/`preferredRegion` 제거) — `unstable_cache`가 신뢰할 수 있게 동작해(nodejs 검증 결과) 대부분의 요청이 60초 캐시를 히트하고, 캐시가 만료되는 시점의 요청 한 번만 리전 왕복+쿼리 비용을 문다. "정확한 리전에서 실행하되 캐시가 매번 깨지는 것"보다 "먼 리전이라도 캐시가 대부분 히트하는 것"이 평균적으로 더 빠르다고 판단.
**검증**: `tsc --noEmit`/`eslint` 통과. 진단용 임시 라우트(`src/app/api/debug/cache-check*`)는 확인 후 삭제.
**영향**: `src/app/page.tsx`. `src/app/feed.xml/route.ts`도 같은 edge+global 설정과 `listArticles`(같은 캐시)를 쓰지만, 이쪽은 응답 자체에 `Cache-Control: public, max-age=60`을 얹어 Vercel CDN이 HTTP 레벨에서 캐시하므로(RSS 구독자 트래픽이라 우선순위도 낮음) 이번엔 손대지 않음 — 나중에 문제되면 같은 방식으로 되돌릴 것.
**다음 단계**: Edge Runtime 자체를 다시 쓰고 싶어지면(리전 왕복을 정말 줄여야 하는 경우), `unstable_cache` 대신 Next.js의 `fetch()` 캐시(edge에서 전역 Data Cache와 확실히 연동되는 경로)로 옮기거나, Supabase 조회 결과를 CDN이 캐시하도록 라우트 자체의 `Cache-Control` 헤더로 얹는 방식을 먼저 검토해야 함 — `unstable_cache`를 edge에서 그대로 재사용하면 오늘 겪은 문제가 그대로 재발한다.
