...
### 2026-08-20 — 일일 수집이 또 절반만 처리됨: 원인 2가지(내가 만든 회귀 + AI 카테고리 기능의 부작용) 특정
**배경**: 사용자가 "또 전체 수집이 안 됐다"고 보고. 프로덕션 `sources`를 조회해보니 32개 중 17개만 2026-08-19 23:32~33 UTC(그날 크론 실행)에 갱신되고 나머지 15개는 전날 05:29~30 UTC(그보다도 더 전 실행에서 처리된 것으로 보임)에 멈춰있음 — 이전에 몇 차례 고쳤던 것과 같은 "일부만 처리되고 체인이 끊김" 증상 재발.
**원인 1 — 08-18에 내가 만든 회귀**: `/api/cron/ingest`의 while 루프가 "다음 배치를 시작해도 되는지"를 배치 시작 **전** 경과시간(`TIME_BUDGET_MS=50_000`)만으로 판단하고, 그 배치가 소스별 상한(`SOURCE_TIMEOUT_MS`, 당시 40_000)을 최악의 경우 전부 소진할 수 있다는 걸 감안하지 않았음. 예: 경과 45초에도 조건을 통과해 배치를 하나 더 시작 → 그 배치가 40초를 다 쓰면 총 85초로 `maxDuration=60`을 넘겨 플랫폼이 함수를 강제 종료(`FUNCTION_INVOCATION_TIMEOUT`, 504) — 응답도 못 보내고 `after()` 폴백조차 실행되지 못한 채 그 실행이 통째로 사라짐. 실제로 이 버그가 있는 상태에서 프로덕션에 수동 curl을 호출해 62초 만에 504가 나는 것으로 재현·확인함.
**수정 1**: "다음 배치를 시작해도 되는 기준"을 `maxDuration − SOURCE_TIMEOUT_MS − 안전 여유`로 낮춰(`TIME_BUDGET_MS`를 35_000으로), 최악의 경우에도 절대 `maxDuration`을 넘기지 않도록 함. `SOURCE_TIMEOUT_MS`도 40_000 → 20_000으로 낮춰(한 실행 안에서 처리 가능한 배치 수를 늘리기 위해 — 이 값이 클수록 안전한 `TIME_BUDGET_MS`가 더 낮아져야 함).
**원인 2 — 어제(2026-08-19) 도입된 AI 카테고리 기능이 크론 소스당 처리시간을 크게 늘림**: `ingestSource()`가 소스에서 가져온 **feed 전체**(대부분 이미 저장된 중복 글)를 매번 `classifyArticlesBatch()`로 OpenAI에 통째로 재분류 요청하고 있었음 — 이 호출의 타임아웃이 `AI_TIMEOUT_MS=20_000`(20초)이라, 실제 API 호출이 오래 걸리면 삽입 작업 시작 전에 최대 20초가 그대로 소스 처리시간에 더해짐. 게다가 **중복 글의 분류 결과는 삽입이 23505로 버려지면서 애초에 한 번도 저장되지 않는 완전한 낭비**였음(신규 글이 없는 평범한 날에도 매번 API를 호출하고 결과를 버림). 이 지연이 위에서 낮춘 `SOURCE_TIMEOUT_MS`(20초)와 거의 같은 크기라, 카테고리 기능이 크론 타임아웃의 실질적인 공동 원인이었을 가능성이 높음.
**수정 2**: `ingestSource()`를 재구성 — 분류 호출을 삽입 **전** feed 전체 대상에서, 삽입 **후** 새로 들어온 글(중복 제외)만 대상으로 옮김. 새 글이 없는 보통 날엔 이 블록이 API 호출 없이 즉시 끝남. 분류는 썸네일 미러링과 병렬로(`Promise.all`) 실행해 순차 대기를 없앰. `category-classifier.ts`의 `AI_TIMEOUT_MS`도 20초 → 12초로 낮춤(이제 호출 대상이 신규 글 몇 건 수준이라 여유를 크론 쪽에 더 남겨줌).
